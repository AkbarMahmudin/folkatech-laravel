<x-app-layout>
  ...
  <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="flex flex-col gap-2">
        ...
        <div class="flex flex-col gap-2 w-full">
          ...
          <input name="firstname" type="text" class="p-2 border-2 border-gray-200/80 rounded" value="{{ $employee->firstname }}">
        </div>

        <div class="flex flex-col gap-2 w-full">
          <label for="lastname" class="font-medium">Lastname</label>
          <input name="lastname" type="text" class="p-2 border-2 border-gray-200/80 rounded" value="{{ $employee->lastname }}">
        </div>

        <div class="flex flex-col gap-2 w-full">
          <label for="email" class="font-medium">Email</label>
          <input name="email" type="email" class="p-2 border-2 border-gray-200/80 rounded" value="{{ $employee->email }}">
        </div>

        <div class="flex flex-col gap-2 w-full">
          <label for="phone" class="font-medium">Phone</label>
          <input name="phone" type="tel" class="p-2 border-2 border-gray-200/80 rounded" value="{{ $employee->phone }}">
        </div>
        ...
        <div class="flex flex-col gap-2 w-full">
          ...
          <select name="company_id" id="" class="p-2 border-2 border-gray-200/80 rounded">
            ...
            @foreach ($companies as $company)
              <option value="{{ $company->id }}" selected="{{ $company->id == $employee->company->id }}">{{ $company->name }}</option>
            @endforeach
          </select>
        </div>
